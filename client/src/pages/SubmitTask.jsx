...
const SubmitTask = () => {
  ...
  const [model, setModel] = useState('qwen'); // 可选模型
  ...
  if (alreadySubmitted) {
    return (
      <div className="p-8 max-w-xl mx-auto">
        <h1 className="text-2xl font-bold mb-4 text-gray-800">提交任务：{task.title}</h1>
        <p className="text-green-600 text-sm">✅ 你已提交此任务，无法重复提交。</p>
        <button
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow p-8">
        ...
        <div className="text-sm text-gray-600 space-y-1 mb-6">
          ...
          <p>⏰ 截止时间：{task.deadline ? new Date(task.deadline).toLocaleDateString() : '未设置'}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block mb-1 font-medium text-gray-700">作业文件（必传）</label>
            <input
              type="file"
